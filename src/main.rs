...
fn ask_recurring_desc<'a>(tasks: &'a [String]) -> PunchDesc {
    ...
    let print_menuline = |key: &str, val: &str| {
        match key.chars().next() {
            Some(chr) if chr.is_alphabetic()  => println!("{:>4}: {}", key, val),
            Some(chr) if chr.is_ascii_digit() => println!("{:>6} {}: {}", "└──", key, val),
            _ => panic!("ERROR: Menuline key '{key}' is neither digit nor letter!"),
        }
    };
